...
from fcore.utils.misc import obtain_cutmix_box
import fcore.utils.distributed as dist_utils
from fcore.optim.utils import sum_loss
from fcore.dataset.build import build_dataloader
from fcore.operators import OPERATOR_REGISTRY, GenericOpt
from fcore.evaluator import EVALUATOR_REGISTRY
from fcore.optim import make_lr_scheduler, make_optimizer
from fcore.dataset.transforms import DataCopy, parse_transform_config
from fcore.utils.classes import CLASSES
...
class TeacherStudentOpt(GenericOpt):
    ...
    def run_test(self) -> Dict:
        ...
        test_results = Dict()
        test_results.stu, _ = self.evaluator_stu.evaluate()
        test_results.tea, iou = self.evaluator_tea.evaluate()
        for (cls_idx, iou) in enumerate(iou):
            logger.info('***** Evaluation ***** >>>> Class [{:} {:}] '
                        'IoU: {:.2f}'.format(cls_idx, CLASSES[self.cfg.data.train_l.dataset][cls_idx], iou * 100))
