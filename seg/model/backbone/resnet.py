...
from seg.model.block import Normalize
from seg.model.block.block import ConvModule
...
class ResNet(GenericModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert depth in self.archs, f"Invaild depth {depth} for ResNet."
        assert num_stages >= 1 and num_stages <= 4, f"Invalid num_stages {num_stages} for ResNet."
        assert (
            len(strides) == len(dilations) == num_stages
        ), f"Length of strides {strides} and dilations {dilations} must maych num_stages {num_stages}."
        ...
        for i, block_num in enumerate(self.stage_block_nums):
            ...
            stage_muti_grid = self.multi_grid if i == len(self.stage_block_nums) - 1 else None 

            planes = self.base_channels * 2 ** i 
            res_layer = self._make_residual_layer(block_num, planes, stride, dilation, stage_muti_grid)
