...
def setup_dist_running(cfg):
    logger = logging.getLogger()

    if "WORLD_SIZE" in os.environ:
        world_size = int(os.environ["WORLD_SIZE"])
    else:
        logger.warning("[!] WORLD_SIZE is not in env, use num_gpus instead.")
        world_size = cfg.get("num_gpus", 1)
    ...
    if world_size > 1:
        try:
            if "MASTER_ADDR" in os.environ:
                ...
                logger.warning("[!] MASTER_ADDR is not in env, use localhost instead.")
                master_addr = "127.0.0.1"

            if "MASTER_PORT" in os.environ:
                master_port = os.environ["MASTER_PORT"]
            else:
                logger.warning("[!] MASTER_PORT is not in env, use a random port instead.")
            ...
            logger.error(
                f"Distributed Initialization Error:\n  URL: {dist_url}\n  World Size: {world_size}\n  Rank: {rank}\n"
            )
            raise e

        synchronize()

        cfg.num_gpus = get_world_size()

        torch.cuda.set_device(f"cuda:{os.environ['LOCAL_RANK']}" if "LOCAL_RANK" in os.environ else "cuda:0")
